...
def clima_varios():
    ...
    for city in listaciudades:
        print ("registrarse en https://home.openweathermap.or ")
...
def bcra():
    ...
    ╚═════════════════════════════════════════════════════════════════════════════╝\033[0;m
    """)
    token="""BEARER eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE3MzExOTA4OTIsInR5cGUiOiJleHRlcm5hbCIsInVzZXIiOiJjdXJzb3MuYWd0QGdtYWlsLmNvbSJ9.YHORLXFZqCFhq4ES_nuri3FJA1qHifAvUskf9TwHl1eflIbETXDKrgI6kwjmNvtZvCQA3CoZ1NJi9Ue33Q156w"""
    puntosfinales = [
                    "usd",
                    "usd_of",
                    "usd_of_minorista",
    ...
    for endpoint in puntosfinales:
        ...
        print (f"{url.center(50)}")
        headers = {"Authorization": token}
        res = requests.get(url, headers=headers)

        print(f"{res.status_code=}   {type(res.status_code)=}")
        if res.status_code != 200:
            print('Respuesta Error de coneccion (no importa cual)')
            exit()
        elif res.status_code==200:
            # ~ print(f"{res.json()=}")
